...
if [[ $EUID -eq 0 ]]; then
    echo "$ERROR This script should not be executed as root!"
    echo "$ACTION Exit"
    exit 1
fi
...
if pacman -Qq | grep -qw pulseaudio; then
    echo "$ERROR PulseAudio is detected as installed"
    exit 1
fi

# Check if base-devel is installed
if pacman -Q base-devel &> /dev/null; then
    echo "$NOTE System: base-devel already installed"
else
    echo "$NOTE Installing: base-devel"

    if sudo pacman -S --noconfirm --needed base-devel; then
        echo "$OK Installed: base-devel"
    else
        echo "$ERROR System: base-devel not found/cannot be installed"
        echo "$ACTION Suggestion: Install base-devel manually before running the script"
        echo "$ACTION Exit"
        exit 1
    fi
fi
...
echo "      /:::/__\:::\    \       /:::/____/              /:::/    /              /:::/|::|   |                  \:::\    \     |::|   |           /:::/__\:::\    \      "
echo "      \:::\   \:::\    \     /::::\    \             /:::/    /              /:::/ |::|   |                  /::::\    \    |::|   |          /::::\   \:::\    \     "
echo "    ___\:::\   \:::\    \   /::::::\    \   _____   /:::/    /      _____   /:::/  |::|___|______   ____    /::::::\    \   |::|___|______   /::::::\   \:::\    \    "
echo "   /\   \:::\   \:::\    \ /:::/\:::\    \ /\    \ /:::/____/      /\    \ /:::/   |::::::::\    \ /\   \  /:::/\:::\    \  /::::::::\    \ /:::/\:::\   \:::\    \   "
...
# Welcome message
echo "$(tput setaf 6)Welcome to Shumiya's Arch-Hyprland Install Script!$(tput sgr0)"
echo
echo "$WARN Run a full system update and Reboot first (Highly Recommended)"
echo
echo "$WARN If you are installing on a VM, ensure 3D acceleration is enabled otherwise Hyprland wont start!"
echo
echo "$NOTE This script offers both automatic and manual installation options"
echo
...
echo "$WARN$(tput bold) Choosing Y on use preset question will install also nvidia stuff"
echo "$WARN$(tput bold) CTRL C to cancel and edit the file preset.sh"
echo "$WARN$(tput bold) If you are not sure what to do, answer N in here"
...
if [[ $use_preset = [Nn] ]]; then
    ...
    printf "\n"
    ask_yes_no "Install XDG-DESKTOP-PORTAL-HYPRLAND? (For proper Screen Share)" xdph
    printf "\n"
    ask_yes_no "Install zsh, omz?" zsh
    printf "\n"
